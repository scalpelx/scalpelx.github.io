...
对每个点i拆为i和i+n，连边(i,i+n,1)，设源点为0，汇点为2\*n+1，然后根据求出的LIS，对每个d[i]==1的点（LIS对应的边的起点），连边(0,i,1)，若d[i]==k(LIS对应的边的终点)，连边(i+n,2\*n+1,1)，对j>i，若d[j]==d[i]+1（即j所在的上升子序列可由i所在的构成），连边(i+n,j,1)。  
现在来分析这样建图的原因，因为所有边的流量为1，所以每个点只会被访问一次，每一条边（即访问顺序为从求解LIS过程中d[i]=1的点一直加1递增到k的点），便对应一个最长上升子序列，这样LIS为k的个数即为图的最大流。  
