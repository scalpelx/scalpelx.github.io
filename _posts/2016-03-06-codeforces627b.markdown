...
**思路**：  
可以利用树状数组来维护修复前后的前缀和，每增加ai订单，就对di执行更新操作，因为修复前后每天的生产量不同，修复前最大为a，修复后最大为b，所以要维护两个不同的前缀和，并且每次增加订单后，当天的生产量不能超过最大的生产量，所以更新时要比较一下。最后输出\[1, pi - 1\]和\[pi + k, n\]这两个最大产量不同的区间和的和即可(注意利用树状数组求区间和时的端点)。  
